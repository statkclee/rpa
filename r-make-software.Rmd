...
유닉스 `Makefile` 자동화 과정을 R언어로 접근해보고, 다양한 작업 흐름의 장단점을 살펴본다.
R과 `make`를 혼합하여 데이터 추출작업, 시각화, 보고서 자동화를 구현한다. 


# 왜 데이터 분석 자동화가 필요한가? [^jtbc-apology] [^reproducible-data-analysis] [^stat545-make] {#r-make}
...
## 재현가능한 데이터 분석 {#r-make-reproducible-research}
...
<img src="fig/make-r-overview.png" alt="자동화 순서" width="100%" />

# 작업 개요 {#r-make-workflow}

페이스북 페이지에서 주요 후보별 팬수를 추출하여 이를 시각화하고 나서 보고서를 생성하는 과정을 자동화한다.

## 모든 작업을 R에서 한방에 처리하는 방법 {#r-make-way}
...
## 쉘스크립트 하나로 자동화하는 방법 {#r-make-shell}
...
## 한통에 `make all` 실행 {#r-make-share-all}
...
## 최종 산출물(html 보고서)을 생성하는 규칙 생성 {#r-make-shell-html}
...
## 최종 산출물(html 보고서)에 대한 의존성(dependency) 추가 {#r-make-shell-dependancy}
...
## 규칙을 모듈화하여 쪼개기 {#r-make-shell-module}
...
# TV 토론이 지지율에 미치는 효과 {#r-make-shell-tv}
...
## JTBC TV 토론 효과 {#r-make-shell-jtbc}
...
DT::datatable(fb_fans_df) %>% 
    DT::formatRound(c(2,3,4), interval = 3, mark = ",", digits = 0)
```
