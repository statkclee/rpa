...
# 데이터 EDA {#autoEDA-data-transform}

`ggplot2::mpg` 데이터에는 결측값이나 문제될 것은 특별히 없고 다만 문자형 변수가 있어 이를 범주형 변수로 변환하는 과정만 거치고 이를 `data` 디렉토리  `mpg_EDA.pdf` 파일로 저장시킨다.
...
ggplot2::mpg %>% 
  ...
  eda_report(target = hwy, output_format = "pdf", output_dir="data", output_file="mpg_EDA.pdf")
```


# 데이터 EDA 보고서 {#autoEDA-data-EDA-report}

![자동차 연비 데이터 EDA 보고서](data/mpg_EDA.pdf){#id .class width="100%" height=600}
